...
public:
   ...
   **/
  RandomNumberGenerator(const int in_seed=-1);
  
  inline int GetUseCount(void) { return use_count; }

  const std::vector<int> &DumpState(void);
  bool LoadState(const std::vector<int> &state);

  /**
   * @return The seed that was actually used to start the random sequence.
   **/
  inline int GetSeed(void){ return seed; }
  
  /**
   * @return The seed that was originally provided by the user.
   **/
  inline int GetOriginalSeed(void){ return original_seed; }
   ...
   **/
  inline double GetDouble(void){ return Get()*_RAND_FAC; }
  ...
  bool Choose(unsigned int num_in, std::vector<unsigned int> &out_array);
   ...
   * @see RandomNumberGenerator::GetRandBinomial
   **/
   ...
   * Generate a random variable drawn from a Binomial distribution.
   * 
   * This function is faster than @ref 
   * RandomNumberGenerator::GetFullRandBinomial(), but 
   * uses some approximations.
   *
   * @see RandomNumberGenerator::GetFullRandBinomial
   **/  
...
private:
  ...
  // Internal functions
  void init(void);	// Setup  (called by ResetSeed(in_seed);
  void initStatFunctions(void);
...
inline UINT RandomNumberGenerator::MutateByte(UINT value) {
...
inline UINT RandomNumberGenerator::ClearByte(UINT value) {
  int byte_pos = 8 * GetUInt(4);
  value &= ~(255 << byte_pos);
  return value;
}

inline UINT RandomNumberGenerator::MutateBit(UINT value) {
  int bit_pos = GetUInt(32);
  value ^= (1 << bit_pos);
  return value;
}

inline UINT RandomNumberGenerator::MutateBit(UINT value, int in_byte) {
